...
function Icon({ name, className = "" }) {
  return <span className={`icon ${name} ${className}`} aria-hidden="true"></span>;
}
      ...
      <div className="product-container">
        ...
        <div className="product-info">
          ...
          <div className="product-services">
            <div><Icon name="truck" /> <strong>Miễn Phí Vận Chuyển:</strong> áp dụng cho đơn trên 500.000₫</div>
            <div><Icon name="sync" /> <strong>Đổi Trả Dễ Dàng:</strong> hỗ trợ đổi trả trong 7 ngày</div>
            <div><Icon name="gift" /> <strong>Quà Quý Tặng:</strong> tặng kèm sticker</div>
          </div>
          ...
          <div className="product-model">
            ...
            <p><CheckIcon /> Chất liệu thân thiện môi trường</p>
            <p><CheckIcon /> Công nghệ in an toàn</p>
          </div>
        </div>
      </div>
...
<div className="review-container">
      ...
      <div className="summary-box">
        <div className="summary-overall">
          <div>Đánh giá tổng thể: <span className="bold">5.0</span></div>
          <div className="stars">
        ...
        <div className="summary-rating-breakdown">
          {[{ width: 90, count: 9 }, { width: 60, count: 6 }, { width: 30, count: 3 }, { width: 10, count: 1 }]
            .map((item, i) => (
              <div key={i} className="rating-row">
                <i className="fas fa-star" />
                <div className="star-bg">
                  <div className="star-fill" style={{ width: `${item.width}%` }}></div>
                </div>
      ...
      {[{
        name: 'Hoàng Huy Tín',
        details: ['Chiều cao: 1m65 - 1m70', 'Cân nặng: 55 - 60 kg', 'Dáng người: Tiêu chuẩn', 'Size đã mua: L', 'Size thường mặc: L'],
        rating: 5,
        title: 'Thoáng mát',
        ...
        details: ['Chiều cao: 1m55 - 1m65', 'Cân nặng: 50 - 55 kg', 'Dáng người: Nhỏ nhắn', 'Size đã mua: M', 'Size thường mặc: M'],
        rating: 5,
        title: 'Siêu thoải mái',
        comment: 'Chất lượng tuyệt vời, ấm áp và cực kỳ thoải mái. Mình chọn size M vì có phần lưng rộng và áo vừa vặn hoàn hảo. Form áo hơi rộng một chút, nhưng điều đó lại rất tốt!',
        time: '14 days ago',
      }].map((review, index) => (
        <div key={index} className="review-card">
          <div className="review-info">
            <div className="review-name bold">{review.name}</div>
            <div className="verified"><i className="fas fa-check-circle" /> Đã Nhận</div>
            {review.details.map((d, i) => <div key={i}>{d}</div>)}
          </div>
          <div className="review-content">
            <div className="stars">{[...Array(review.rating)].map((_, i) => <i key={i} className="fas fa-star" />)}</div>
      ...
      <div className="price-note">
        Chúng tôi công khai chi phí sản xuất từng sản phẩm để bạn hiểu rõ hơn về giá trị thực sự. Một số chi phí khác như thiết kế,
        thử nghiệm, kiểm tra chất lượng, thuê văn phòng và cửa hàng không được liệt kê đầy đủ, nhưng chúng tôi tin rằng bạn xứng đáng được biết
        những gì tạo nên sản phẩm mà bạn yêu thích. 
      </div>
  ...
  {[
    ...
    { label: 'Thuế & phí phát', value: '10.5%', icon: 'file-invoice-dollar' },
